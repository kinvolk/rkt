...
type testParser struct {
	linesPerDir map[*PathIndex][]string
}

func newTestParser() *testParser {
	return &testParser{
		linesPerDir: make(map[*PathIndex][]string),
	}
}

func (p *testParser) reset() {
	p.linesPerDir = make(map[*PathIndex][]string)
}

func (p *testParser) Parse(idx *PathIndex, raw []byte) error {
	s := getScanner(raw)
	...
	p.linesPerDir[idx] = append(p.linesPerDir[idx], lines...)
	return nil
}
...
func TestInvalidDirectories(t *testing.T) {
	tmp, err := ioutil.TempDir("", "rkt-config-invalid-directory")
	if err != nil {
...
func TestValidConfig(t *testing.T) {
	tmp, err := ioutil.TempDir("", "rkt-config-valid")
	if err != nil {
		t.Fatalf("failed to create a temporary directory: %v", err)
	}
	defer os.RemoveAll(tmp)
	...
	for i, tt := range tests {
		...
		if tt.success {
			for pi, gotLinesInIndex := range parser.linesPerDir {
				t.Logf("Checking lines from %q", pi.Path)
				if len(tt.linesPerIndex) <= pi.Index {
					t.Errorf("expected to parse only %d directories, but apparently we parsed %d (unplanned directory is %q)", len(tt.linesPerIndex), pi.Index+1, pi.Path)
					continue
				}
				expectedLinesInIndex := tt.linesPerIndex[pi.Index]
				...
				for _, line := range gotLinesInIndex {
...
func TestParsing(t *testing.T) {
	tmp, err := ioutil.TempDir("", "rkt-config-parsing")
	if err != nil {
		t.Fatalf("failed to create a temporary directory: %v", err)
	}
	defer os.RemoveAll(tmp)

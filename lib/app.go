...
// GetApps calls the stage1 entrypoing for 'app/status' and parses the json result.
// If 'appName' is non-empty, then the returned list will only contain the app with the given name.
func GetApps(dataDir, uuid string, appName string) ([]*App, error) {
	p, err := GetPodByFullUUID(dataDir, uuid)
	if err != nil {
		return nil, err
	}
	defer p.Close()

	podManifest, err := p.GetPodManifest()
	if err != nil {
		return nil, err
	}
...
func getApp(ra *schema.RuntimeApp, podManifest *schema.PodManifest, pod *Pod) (*App, error) {
	app := &App{
		Name:        ra.Name.String(),
...
func getAppState(app *App, pod *Pod) error {
	app.State = AppStateUnknown

	appCreatedFile := appCreatedFile(app.Name, pod.path())
	appStartedFile := appStartedFile(app.Name, pod.path())
	appExitedFile := appExitedFile(app.Name, pod.path())

	defer func() {
		if pod.AfterRun() { // If the pod is hard killed, set the app to 'exited' state.
			app.State = AppStateExited
		}
	}()

	// Check if the app is created.
	fi, err := os.Stat(appCreatedFile)
	if err != nil {
...
func appStatusDir(appName, podDir string) string {
	return filepath.Join(common.Stage1RootfsPath(podDir), "rkt", "status")
}

func appCreatedFile(appName, podDir string) string {
	return filepath.Join(appStatusDir(appName, podDir), fmt.Sprintf("%s-created", appName))
}

func appStartedFile(appName, podDir string) string {
	return filepath.Join(appStatusDir(appName, podDir), fmt.Sprintf("%s-started", appName))
}

func appExitedFile(appName, podDir string) string {
	return filepath.Join(appStatusDir(appName, podDir), appName)
}

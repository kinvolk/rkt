...
func TestStage1LoadingFromConfig(t *testing.T) {
	...
	tests := []struct {
		...
		// store on subsequent runs
		{
			...
			expectedFirstRun:  fmt.Sprintf("rkt: using image from file %s", setup.getLocation(stubStage1PathAbs)),
			expectedSecondRun: fmt.Sprintf("rkt: using image from local store for image name %s:%s", setup.name, setup.version),
		},
		...
		// image from disk
		{
			...
			location:          setup.getLocation(stubStage1PathAbs),
			expectedFirstRun:  fmt.Sprintf("rkt: using image from file %s", setup.getLocation(stubStage1PathAbs)),
			expectedSecondRun: fmt.Sprintf("rkt: using image from file %s", setup.getLocation(stubStage1PathAbs)),
		},
		...
		// loading stage1 from https URL
		{
			...
			expectedFirstRun:  fmt.Sprintf("rkt: remote fetching from URL %q", setup.getLocation(stubStage1UrlHttps)),
			expectedSecondRun: fmt.Sprintf("rkt: using image from local store for image name %s:%s", setup.name, setup.version),
		},
...
func TestStage1LoadingFromConfigHttp(t *testing.T) {
	...
	setup := newStubStage1Setup(t, serverSetup)
	defer setup.cleanup()
	expectedFirstRun := fmt.Sprintf("rkt: remote fetching from URL %q", setup.getLocation(stubStage1UrlHttp))
	expectedSecondRun := fmt.Sprintf("rkt: using image from local store for image name %s:%s", setup.name, setup.version)
	cfg := &config.Stage1Data{
		Name:     setup.name,
		Version:  setup.version,
...
func TestStage1LoadingFromConfigFallback(t *testing.T) {
	...
	setup.getExpectedOrFail(child, fmt.Sprintf("rkt: using image from file %s", fakePath))
	setup.getExpectedOrFail(child, fmt.Sprintf("rkt: using image from file %s", setup.getLocation(stubStage1PathAbs)))
...
func TestStage1LoadingFromConfigNoDiscovery(t *testing.T) {
	...
	for {
		matches, output, err := expectRegexWithOutput(child, `(?m)^rkt:.+$`)
		if err != nil {
...
func TestStage1LoadingFromFlags(t *testing.T) {
	...
	tests := []struct {
		...
		// --stage1-path with a relative path
		{
			...
			expectedFirstRun:  fmt.Sprintf("rkt: using image from file %s", setup.getLocation(stubStage1PathRel)),
			expectedSecondRun: fmt.Sprintf("rkt: using image from file %s", setup.getLocation(stubStage1PathRel)),
		},
		...
		// --stage1-url with an https URL
		{
			...
			expectedFirstRun:  fmt.Sprintf("rkt: remote fetching from URL %q", setup.getLocation(stubStage1UrlHttps)),
			expectedSecondRun: fmt.Sprintf("rkt: using image from local store for url %s", setup.getLocation(stubStage1UrlHttps)),
		},
		...
		// --stage1-name
		{
			...
			expectedFirstRun:  fmt.Sprintf("rkt: searching for app image %s", setup.name),
			expectedSecondRun: fmt.Sprintf("rkt: using image from local store for image name %s", setup.getLocation(stubStage1Name)),
		},
...
func TestStage1LoadingFromFlagsHttp(t *testing.T) {
	...
	expectedFirstRun := fmt.Sprintf("rkt: remote fetching from URL %q", setup.getLocation(stubStage1UrlHttp))
	expectedSecondRun := fmt.Sprintf("rkt: using image from local store for url %s", setup.getLocation(stubStage1UrlHttp))
...
func TestStage1LoadingFromFlagsFromDir(t *testing.T) {
	...
	child := spawnOrFail(setup.t, cmd)
	defer waitOrFail(setup.t, child, true)
	setup.getExpectedOrFail(child, fmt.Sprintf("rkt: using image from file %s", stubCopyPath))
	setup.getExpectedOrFail(child, stubStage1Output)
}

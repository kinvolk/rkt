...
func TestImageNameToAppName(t *testing.T) {
	for _, tt := range []struct {
		...
		{
			in:  types.ACIdentifier("https://github.com/coreos/etcd/releases/download/v2.0.0/etcd-v2.0.0-linux-amd64.aci"),
			out: types.ACName("etcd-v2-0-0-linux-amd64-aci"),
			err: false,
		},
		{
			in:  types.ACIdentifier("docker://registry.hub.docker.com/library/fedora:26"),
			out: types.ACName("fedora-26"),
			err: false,
		},
		{
